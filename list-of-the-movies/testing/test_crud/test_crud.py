...
def test_create_of_raise_if_exists() -> None:
    existing_movie = create_movie()
    created_movie = CreateMovie(**existing_movie.model_dump())
    with pytest.raises(
        expected_exception=AlreadyExistsError, match=existing_movie.slug
    ) as ex_info:
